<template>
	<view class="u-subsection" :style="[subsectionStyle]">
		<view class="u-item" :style="[itemStyle(index)]" @tap="click(index)" :class="[noBorderRight(index), 'u-item-' + index]" v-for="(item, index) in listInfo" :key="index">
			<view :style="[textStyle(index)]" class="u-item-text">{{item.name}}</view>
		</view>
...
<script>
	export default {
		props: {
			...
			// 模式选择，mode=tabs为带下划线形式，mode=button为按钮形式，mode=subsection时为分段模式
			mode: {
				type: String,
				default: 'button'
			},
		...
		data() {
			return {
				...
				tabWidth: uni.upx2px(this.barWidth), // mode = tabs模式下，滑块的宽度，单位px
		...
		methods: {
			...
			changeSectionStatus(nVal) {
				if(this.mode == 'subsection') {
					...
					this.itemBgStyle.borderRadius = '100rpx';
				}
				// 更新滑块的位置
				setTimeout(() => {this.itemBgLeft();}, 10);
			...
			getTabsInfo() {
				...
				view.exec(res => {
					...
					if(this.mode == 'subsection') {
						...
						this.itemBgStyle.width = this.tabWidth + 'px';
					} else if(this.mode == 'button') {
						this.itemBgStyle.width = this.listInfo[0].width + 'px';
					}
					// 初始化滑块的位置
			...
			itemBgLeft() {
				...
				// 根据mode不同模式，计算滑块需要移动的距离，如果mode=tabs，需要另外计算，把滑块移动到当前激活item中间
				if(this.mode == 'subsection') {
					this.itemBgStyle.left = left + 'px';
				} else if(this.mode == 'tabs') {
					this.itemBgStyle.left = left + this.listInfo[this.currentIndex].width / 2 - this.tabWidth / 2 + 'px';
				} else if(this.mode == 'button') {
