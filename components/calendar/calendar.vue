		...
		<view class="calendar-layer" :animation="animationData" :class="isShow_H5 ? 'show' : 'hide'">
			...
			<view class="layer-content" :class="{ choiceDate: choice === true || singleDate }">
				...
				<view class="order-time" @tap="showCalendar()" v-if="!modal">
					...
					<view class="time-viewer zt-time-viewer" v-else>
						<view>
							<view class="goInHotel">入住</view>
							<view>
								<view class="date-wrappper">{{ choiceDate[0].month }}月{{ choiceDate[0].day }}日</view>
								<view class="goInHotel2" v-if="choiceDate[0].year + '' + choiceDate[0].month + '' + choiceDate[0].day == today">今天</view>
							</view>
						</view>
						<view class="sumCount">({{ dayCount2 }})</view>
						<view>
							<view class="left-hotel">离店</view>
							<view>
								<view class="date-wrappper">{{ choiceDate[1].month }}月{{ choiceDate[1].day }}日</view>
								<view class="goInHotel2" v-if="choiceDate[choiceDate.length - 1].year + '' + choiceDate[choiceDate.length - 1].month + '' + choiceDate[choiceDate.length - 1].day == tomorrow">
									明天
								</view>
							</view>
						</view>
